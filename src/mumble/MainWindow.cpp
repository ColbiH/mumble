...
void MainWindow::closeEvent(QCloseEvent *e) {
	...
	if (sh && sh->isRunning() && Global::get().s.bMinimizeOnQuit && !bSuppressAskOnQuit && !Global::get().s.bAskOnQuit){
		showMinimized();
		return;
	} else if (sh && sh->isRunning() && Global::get().s.bCloseOnQuit && !bSuppressAskOnQuit && !Global::get().s.bAskOnQuit){
		sh.reset();
		return;
	} else if (sh && sh->isRunning() && Global::get().s.bAskOnQuit && !bSuppressAskOnQuit) {
		//Reset Booleans for Minimize and Close on Quit
		Global::get().s.bCloseOnQuit = false;
		Global::get().s.bMinimizeOnQuit = false;
		...
		QCheckBox *qcbRemember   = new QCheckBox(tr("Remember this Preference?"));
		mb.setCheckBox(qcbRemember);
		mb.setDefaultButton(qpbClose);
		mb.setEscapeButton(qpbCancel);
		QObject::connect(qcbRemember, &QCheckBox::stateChanged, [this](int state) {
			if (static_cast< Qt::CheckState >(state) == Qt::CheckState::Checked) {
				Global::get().s.bAskOnQuit = false;
			} else {
				Global::get().s.bAskOnQuit = true;
			}
		});
		mb.exec();
		if (mb.clickedButton() == qpbMinimize) {
			if (qcbRemember->isChecked()) {
				Global::get().s.bMinimizeOnQuit = true;
			}
			showMinimized();
			e->ignore();
			return;
		} else if (mb.clickedButton() == qpbCancel) {
			// Removes affect of booleans if Remember box was checked.
			if (qcbRemember->isChecked()) {
				Global::get().s.bAskOnQuit = true;
			}
			e->ignore();
		...
		if (mb.clickedButton() == qpbClose) {
			if (qcbRemember->isChecked()) {
				Global::get().s.bCloseOnQuit = true;
			}
		}
	}
	sh.reset();
